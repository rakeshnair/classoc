...
*/
countertype Input_Output_Manager::find_frequent_items( 
   const double min_supp, vector<countertype>& support_of_items )
{
   countertype basket_number = 0;
   set<itemtype> basket;
   vector< countertype > temp_counter_vector;
   ...
   for( edgeIndex = 0; edgeIndex < temp_counter_vector.size(); 
	edgeIndex++ )
      if( temp_counter_vector[edgeIndex] > min_occurrence )
	 temp_set.insert(
	    pair<countertype, itemtype>(temp_counter_vector[edgeIndex],
					  edgeIndex));

   new_code_inverse.clear();
   support_of_items.clear();
   for(set< pair<countertype, itemtype> >::iterator it = temp_set.begin();
       it != temp_set.end(); it++)
   {
...
//   reverse( support_of_items.begin(), support_of_items.end() );
   ...
   vector<countertype>(support_of_items).swap(support_of_items);
...
{
   new_basket.clear();
   for( set<itemtype>::const_iterator it_basket = original_basket.begin(); 
...
void Input_Output_Manager::write_out_basket_and_counter(
   const set<itemtype>& itemset, const countertype counter)
{
